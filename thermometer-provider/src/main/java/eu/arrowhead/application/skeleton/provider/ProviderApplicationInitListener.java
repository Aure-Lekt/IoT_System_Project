...
public class ProviderApplicationInitListener extends ApplicationInitListener {
	...
	protected void customInit(final ContextRefreshedEvent event) {
		...
		ServiceRegistryRequestDTO getTempServiceRequest = createServiceRegistryRequest(TemperatureProviderConstants.GET_TEMP_SERVICE_DEFINITION,  TemperatureProviderConstants.TEMP_URI, HttpMethod.GET);
		getTempServiceRequest.getMetadata().put(TemperatureProviderConstants.REQUEST_PARAM_KEY_TIME, TemperatureProviderConstants.REQUEST_PARAM_TIME);
		getTempServiceRequest.getMetadata().put(TemperatureProviderConstants.REQUEST_PARAM_KEY_VALUE, TemperatureProviderConstants.REQUEST_PARAM_VALUE);
	...
	public void customDestroy() {
		//Unregister service
		//arrowheadService.unregisterServiceFromServiceRegistry(TemperatureProviderConstants.CREATE_TEMP_SERVICE_DEFINITION, TemperatureProviderConstants.TEMP_URI);
		arrowheadService.unregisterServiceFromServiceRegistry(TemperatureProviderConstants.GET_TEMP_SERVICE_DEFINITION, TemperatureProviderConstants.TEMP_URI);
	}
	...
	private ServiceRegistryRequestDTO createServiceRegistryRequest(final String serviceDefinition, final String serviceUri, final HttpMethod httpMethod) {
		...
		if (sslEnabled && tokenSecurityFilterEnabled) {
			...
			serviceRegistryRequest.setInterfaces(List.of(TemperatureProviderConstants.INTERFACE_SECURE));
		} else if (sslEnabled) {
			systemRequest.setAuthenticationInfo(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
			...
			serviceRegistryRequest.setInterfaces(List.of(TemperatureProviderConstants.INTERFACE_INSECURE));
		}
		serviceRegistryRequest.setProviderSystem(systemRequest);
		serviceRegistryRequest.setServiceUri(serviceUri);
		serviceRegistryRequest.setMetadata(new HashMap<>());
		serviceRegistryRequest.getMetadata().put(TemperatureProviderConstants.HTTP_METHOD, httpMethod.name());
